...
\todo{Put in here typical usage examples the can be copied/pasted and ready
to be used. I think it would be cool to have this for all the variants given
in \Cref{sec:resource-allocation}}.

Split the message to be hashed into chunks of \emph{BLOCK_SIZE} bytes. Send
them to the entity sequentially by setting the message port and raising
\emph{valid_in} for one clock cycle.
...
\begin{table}[tb]
	...
	\centering
	\caption{User interface for the BLAKE2 cores}	
	\label{tbl:blake2-ui}
	...
	& Input
	& ---
	&	When `HIGH', the data on the \emph{message} input is processed
	\\\hline
...
64-bit words from message $m_i$ into hash state $h_i$. We an use auxiliary
...
\begin{table}[tb]
	...
	\begin{tabular}{l|l|c}
	\hline
	\textbf{Input message} & \textbf{Hash} & \textbf{Hash length} \\
